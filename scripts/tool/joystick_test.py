...
def load_config():
        ...
        try:
            ...
            if isinstance(jc, dict):
                for key in ("linear_axis", "linear_direction", "angular_axis",
                            "angular_direction", "dead_zone", "sat_zone"):
                    if key in jc:
                        cfg[key] = float(jc[key]) if "direction" in key or "zone" in key else int(jc[key])
    ...
    if base_path:
        try:
            ...
            jc = (data.get("joy_control", {}) or {}).get("ros__parameters", {}) or {}
            if isinstance(jc, dict):
                for key, target in (("js_vel_high", "vel_high"),
...
def main():
    ...
    print(f"  死区={dead_zone}  饱和区={sat_zone}")
    print(f"  最大速度: {vel_high}m/s  {rot_high}°/s")
    print("=" * 52)
    print()
    print("  物理轴位(左:前后 右:旋转)  方向          线速度        角速度      原始轴值")
    print("  " + "-" * 48)
    ...
    try:
        while True:
            ...
            lin_speed = lin_norm * vel_high
            ang_speed_rad = ang_norm * math.radians(rot_high)
