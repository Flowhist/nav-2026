...
void XingsongDriver::Init()
{
    ...
    {
        scan_pub = node_->create_publisher<sensor_msgs::msg::LaserScan>(
...
sensor_msgs::msg::LaserScan XingsongDriver::ToLaserScan(const ScanData &data, float start_angle, float end_angle, std::string frame_name)
{
 ...
 sensor_msgs::msg::LaserScan msg;
  ...
  unsigned long long system_time_stamp =
    static_cast<uint64_t>(receive_time.nanoseconds());
  double scan_period = 0.0;
  if (last_system_time_stamp > 0 && system_time_stamp > last_system_time_stamp) {
    const double measured_period =
      static_cast<double>(system_time_stamp - last_system_time_stamp) * 1.e-9;
    if (measured_period >= 0.005 && measured_period <= 0.5) {
      scan_period_sec_ = scan_period_sec_ == 0.0 ? measured_period :
        0.9 * scan_period_sec_ + 0.1 * measured_period;
    }
  }
  scan_period = scan_period_sec_;
  ...
  if(synctype)
  {
    ...
    if (msg.header.stamp.sec == 0 && msg.header.stamp.nanosec == 0) {
      msg.header.stamp = receive_time;
    } else {
      system_time_stamp =
        static_cast<unsigned long long>(msg.header.stamp.sec) * 1000000000ULL +
        static_cast<unsigned long long>(msg.header.stamp.nanosec);
    }
  }
  else
  {
    const double acquisition_time =
      scan_period * static_cast<double>(end_laser_angle - start_laser_angle) / 360.0;
    msg.header.stamp = receive_time - rclcpp::Duration::from_seconds(acquisition_time);
  }
  ...
  const double acquisition_time =
    scan_period * static_cast<double>(end_laser_angle - start_laser_angle) / 360.0;
  msg.time_increment = data.distance_data.size() > 1 ?
    acquisition_time / static_cast<double>(data.distance_data.size() - 1) : 0.0;
  ...
  msg.angle_increment = size > 1 ? detaangle / static_cast<float>(size - 1) : 0.0f;
...
void XingsongDriver::ntp_to_ros2_stamp(uint64_t ntptim, sensor_msgs::msg::LaserScan &msg)
{
    ...
    // 填充 LaserScan 的时间戳
    msg.header.stamp.sec = static_cast<int32_t>(unix_seconds);
    msg.header.stamp.nanosec = nanoseconds;
    last_unix_seconds = msg.header.stamp.sec;
    last_nanoseconds = msg.header.stamp.nanosec;
